...
<p class="text-muted">
    <?php if ($isRevisitingSkipped): ?>
        Skipped Question <span id="current-question-overall-number"
            class="fw-semibold text-primary"><?= $overallQuestionNumber ?></span>
        <span class="badge bg-warning text-dark ms-2">Revisiting (<?= $currentPhaseIndex ?> of
            <?= $totalQuestionsInPhase ?>)</span>
    <?php else: ?>
        Question <span id="current-question-overall-number"
            class="fw-semibold text-primary"><?= $overallQuestionNumber ?></span>
